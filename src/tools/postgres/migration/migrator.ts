...
export class Migrator {
  ...
  constructor(
    ...
    if (!logger) {
      this.logger = new Logger(Migrator.name);
    } else {
      this.logger = logger;
    }
  }
  ...
  async up() {
    this.logger.verbose('start UP migrations');
    ...
    this.logger.verbose('migrations finished');
  }

  async down(full = false) {
    this.logger.verbose('start DOWN migrations');
    const executedMigrationNames = await this.getExecutedMigrations().then(
